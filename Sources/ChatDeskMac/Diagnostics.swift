...
extension MainWindowController {
    ...
    func exportDiagnostics() {
        ...
        let panel = NSSavePanel(); panel.nameFieldStringValue = "ChatPretzel-diagnostics.json"
        panel.beginSheetModal(for: window) { [weak self] response in
            guard response == .OK, let url = panel.url else { return }
            do {
                let data = try JSONSerialization.data(withJSONObject: report, options: [.prettyPrinted, .sortedKeys])
                try data.write(to: url, options: .atomic); self?.notice("Technical diagnostics saved without chat content or filenames.")
            } catch { self?.notice("The diagnostics file could not be saved.") }
        }
    }
}
