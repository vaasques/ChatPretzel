...
final class DownloadCoordinator: NSObject, WKDownloadDelegate {
    ...
    func accept(_ download: WKDownload) {
        ...
        guard active.count < 8 else {
            download.cancel { _ in }; onNotice?("Högst åtta samtidiga nedladdningar. Den nya nedladdningen avbröts."); return
        }
        if rows.count >= 100 { rows.removeAll { active[$0.id] == nil } }
        active[key] = download; download.delegate = self
        rows.append(Row(id: key, name: "Nedladdning", state: "Väntar på sparplats"))
    ...
    private func presentNextChoice() {
        ...
        guard active[key] != nil, let window, window.attachedSheet == nil else {
            completion(nil); change(key, name: name, state: "Avbruten – sparpanelen kunde inte öppnas")
            active.removeValue(forKey: key); presentNextChoice(); return
        }
        presenting = true
        let panel = NSSavePanel()
        panel.title = "Spara nedladdning"
        panel.message = "Välj ett nytt filnamn. ChatPretzel skriver inte över befintliga filer."
        ...
        panel.beginSheetModal(for: window) { [weak self] response in
            ...
            guard response == .OK, let destination = panel.url, self.active[key] != nil else {
                completion(nil); self.change(key, name: name, state: "Avbruten")
            ...
            guard !FileManager.default.fileExists(atPath: destination.path) else {
                completion(nil); self.change(key, name: name, state: "Avbruten – filnamnet finns redan")
                self.active.removeValue(forKey: key)
                self.onNotice?("Filen finns redan. Starta nedladdningen igen och välj ett annat namn.")
                self.presentNextChoice(); return
            }
            self.destinationLeases[key] = FileAccessLease(destination)
            self.change(key, name: destination.lastPathComponent, state: "Laddar ned", destination: destination)
            completion(destination); self.presentNextChoice()
        }
    }
    func downloadDidFinish(_ download: WKDownload) {
        let key = ObjectIdentifier(download)
        change(key, state: "Sparad", finished: true)
        active.removeValue(forKey: key); destinationLeases.removeValue(forKey: key)
        onNotice?("Nedladdningen är sparad. Se Nedladdningar för Visa i Finder.")
    }
    func download(_ download: WKDownload, didFailWithError error: Error, resumeData: Data?) {
        let key = ObjectIdentifier(download)
        // Do not log NSError.userInfo or resumeData: they may contain signed URLs or tokens.
        change(key, state: "Misslyckades (\((error as NSError).code))")
        active.removeValue(forKey: key); destinationLeases.removeValue(forKey: key)
    }
    func cancel(_ keys: Set<ObjectIdentifier>) {
        for key in keys {
            ...
            active.removeValue(forKey: key); destinationLeases.removeValue(forKey: key)
            change(key, state: "Avbruten – eventuell delvis fil lämnas orörd")
        }
    }
