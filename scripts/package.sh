...
VERSION="$(/usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' dist/ChatPretzel.app/Contents/Info.plist)"
OUT="$ROOT/dist/ChatPretzel-$VERSION-$(uname -m)-$(date +%Y%m%d-%H%M%S).dmg"
