...
- Super Upload for selections larger than 10 files, using native Swift queueing and batches of 10
...
For more than 10 files pasted, dropped, or selected through the app's file chooser, ChatPretzel asks **Start batch upload?** Choose **Yes** (or press **Enter**) to authorise the entire queue, including the first message. Choose **No** to leave the files and message unsent. ChatPretzel checks the files locally, skips unreadable or unsupported items, divides the remaining files into batches of 10, and adds a short progress note below your existing instruction. It waits for ChatGPT to finish each response before sending the next batch. The notes use Swedish for a Swedish instruction or chat and English otherwise; the Mac's preferred language is used when the chat provides no language signal.

Write your instruction before starting the upload. Once you choose Yes, the first batch sends automatically when ready. Files with matching names are kept: later copies receive names such as `report 2.pdf` and `report 3.pdf` for upload. Your originals are never renamed. Temporary copies use filesystem cloning when available and are removed after their batch is confirmed, or when the operation ends or is cancelled.

After Yes, a frosted progress screen locks the conversation against accidental changes. It displays `Uploading in progress`, the number of files processed, and **Cancel**. You can use other apps or minimise ChatPretzel while the queue runs; keep ChatPretzel open. During an active queue, the app temporarily prevents App Nap and idle system sleep; on macOS 14 or later it also disables WebKit's inactive-page suspension. Normal scheduling is restored when the queue finishes or stops. Manually putting the Mac to sleep, closing its lid, or losing the connection can still interrupt uploads.

The lock clears after the final response finishes or when Cancel is pressed. Clearly identified failed attachments are skipped and the totals are updated; ambiguous errors stop the queue with an explanation. ChatPretzel checks attachment cards and sent messages instead of assuming that an empty composer means success. If ChatGPT's interface does not expose enough evidence, the queue stops rather than claiming completion. Cancellation does not retract messages already sent.
